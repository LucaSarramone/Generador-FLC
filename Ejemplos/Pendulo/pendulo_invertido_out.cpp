...
void fuzzifier (fixed_int angulo, fixed_int velocidad) { 
 #pragma HLS PIPELINE 
 #pragma HLS ARRAY_PARTITION variable=anguloFuzz complete dim=1 
 #pragma HLS ARRAY_PARTITION variable=velocidadFuzz complete dim=1 
...
void rulesEvaluation(){ 
 ...
 #pragma HLS ARRAY_PARTITION variable=anguloFuzz complete dim=1 
 #pragma HLS ARRAY_PARTITION variable=velocidadFuzz complete dim=1 
 #pragma HLS ARRAY_PARTITION variable=fuerzaMembershipValues complete dim=1 
...
fixed_int defuzzifierfuerza() {
 #pragma HLS PIPELINE 
 #pragma HLS ARRAY_PARTITION variable=fuerzaMembershipValues complete dim=1 
 #pragma HLS ARRAY_PARTITION variable=outputValuesfuerza complete dim=1 
...
fixed_out fuzzyController(fixed_int angulo, fixed_int velocidad){ 
 #pragma HLS DATAFLOW 
	...
	fixed_out output0 = defuzzifierfuerza();
	return output0;
}
